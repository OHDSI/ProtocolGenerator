...
if(params$addCohortDefinitions){
  ...
  parentCohortsInd <- which(cohortDefinitionDf$isParent)
...
# Then display the subset logic with link to subsetDef section containing subsetUnique
  subCohortInds <- setdiff(which(cohortDefinitionDf$parentId == cohortDefinitions[[i]]$cohortId), i)
  
  if(length(subCohortInds) >0){
    
  # order the subCohorts
  subnames <- cohortDefinitionDf$cohortName[subCohortInds]
