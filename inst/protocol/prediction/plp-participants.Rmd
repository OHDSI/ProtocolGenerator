...
cat('The target population ', cohortNames[which(modelDesignSetting$targetId == cohortIds)],' see ', paste0('@sec-',which(modelDesignSetting$targetId == cohortIds)), ' for cohort details. \\n')
```
...
if(!is.null(cohortDefinitions[[which(modelDesignSetting$targetId == cohortIds)]]$cohortDefinition)){
  if(washout > ParallelLogger::convertJsonToSettings(cohortDefinitions[[which(modelDesignSetting$targetId == cohortIds)]]$cohortDefinition)$PrimaryCriteria$ObservationWindow[1]){
